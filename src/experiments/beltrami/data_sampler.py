        ...
        if self.update_data:
            ...
            uvwp_bi = torch.stack(
                [
                    ...
                    p_func_torch(x_bi, y_bi, z_bi, t_bi),
                ],
                dim=-1,
            )
...
class BeltramiSamplerLH(BeltramiSamplerBase):
        ...
        with torch.no_grad():
            ...
            y_length_boundary = torch.cat(
                [
                    ...
                    self.y_start
                    * torch.ones(
                        self.n_length - self.n_length // 4 - self.n_length // 2,
                        device=self.device,
                    ),
                ]
            )
            z_length_boundary = torch.cat(
                [
                    ...
                    self.z_end
                    * torch.ones(
                        self.n_length - self.n_length // 2, device=self.device
                    ),
                ]
            )
            ...
            x_width_boundary = torch.cat(
                [
                    ...
                    self.x_start
                    * torch.ones(
                        self.n_width - self.n_width // 4 - self.n_width // 2,
                        device=self.device,
                    ),
                ]
            )
            ...
            z_width_boundary = torch.cat(
                [
                    ...
                    self.z_end
                    * torch.ones(self.n_width - self.n_width // 2, device=self.device),
                ]
            )
            ...
            x_height_boundary = torch.cat(
                [
                    ...
                    self.x_start
                    * torch.ones(
                        self.n_height - self.n_height // 4 - self.n_height // 2,
                        device=self.device,
                    ),
                ]
            )
            y_height_boundary = torch.cat(
                [
                    ...
                    self.y_end
                    * torch.ones(
                        self.n_height - self.n_height // 2, device=self.device
                    ),
                ]
            )
            z_height_boundary = (
    ...
    def _sample_initial(self, return_uvwp: bool = True):
        with torch.no_grad():
            ...
            z = (
                torch.tensor(sample[:, 2], device=self.device, dtype=torch.float32)
                * (self.z_end - self.z_start)
                + self.z_start
            )
    ...
    def _sample_internal(self):
        with torch.no_grad():
            ...
            t = (
                torch.tensor(sample[:, 3], device=self.device, dtype=torch.float32)
                * self.simulation_time
            )
...
class BeltramiSamplerMC(BeltramiSamplerBase):
    ...
    def _get_boundary_points(self):
        with torch.no_grad():
            x_length_boundary = (
                torch.rand(self.n_length, device=self.device)
                * (self.x_end - self.x_start)
                + self.x_start
            )
            y_length_boundary = torch.cat(
            ...
            y_width_boundary = (
                torch.rand(self.n_width, device=self.device)
                * (self.y_end - self.y_start)
                + self.y_start
            )
            z_width_boundary = torch.cat(
            ...
            z_height_boundary = (
                torch.rand(self.n_height, device=self.device)
                * (self.z_end - self.z_start)
                + self.z_start
            )
            x_b = torch.cat([x_length_boundary, x_width_boundary, x_height_boundary])
...
class BeltramiValidationDataSet:
    ...
    def __init__(
        ...
        n_point_x = int(
            np.power(n_point / (y_length / x_length) / (z_length / x_length), 1 / 3)
        )
        ...
        self.uvwp = np.stack(
            [
                ...
                p_func_np(self.x, self.y, self.z, self.t),
            ],
            axis=-1,
        )
...
class BeltramiValidationDataLoader:
    ...
    def __init__(
        self,
        validation_dataset: BeltramiValidationDataSet,
        device: Union[str, torch.device] = "cuda:0",
    ) -> None:
        self.fake_data = [0]
