...
from src.experiments.kovasznay.data_sampler import KovasznayValidationDataSet
...
def run_test(
    ...
    network.eval()
    network.to(device)
    dataset = KovasznayValidationDataSet(
        x_start=x_start, x_end=x_end, y_start=y_start, y_end=y_end, n_point=n_point
    )
    with torch.no_grad():
        prediction = network(
