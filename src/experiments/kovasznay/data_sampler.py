...
def p_func_np(
    ...
    y: Optional[Union[float, Sequence]] = None,
    wave_length: float = LAMBDA,
):
    return 0.5 * (1 - np.exp(2 * wave_length * x))


def v_func_np(
    x: Union[float, Sequence], y: Union[float, Sequence], wave_length: float = LAMBDA
):
...
def p_func_torch(
    x: torch.Tensor, y: Optional[torch.Tensor] = None, wave_length: float = LAMBDA
):
...
class KovasznaySamplerBase:
    ...
    def __init__(
        ...
        self.n_boundary_top_bottom = int(
            n_boundary / (x_end - x_start + y_end - y_start) * (x_end - x_start)
        )
...
class KovasznaySamplerLH(KovasznaySamplerBase):
    ...
    def _sample_boundary(self):
        with torch.no_grad():
            x_top_bottom = (
                torch.tensor(
                    self.random_engine_boundary.random(n=self.n_boundary_top_bottom)[
                        :, 0
                    ],
                    device=self.device,
                    dtype=torch.float32,
            ...
            y_top_bottom = torch.cat(
                [
                    self.y_start
                    * torch.ones(self.n_boundary_top_bottom // 2, device=self.device),
                    self.y_end
                    * torch.ones(
                        self.n_boundary_top_bottom - self.n_boundary_top_bottom // 2,
                        device=self.device,
                    ),
                ]
            )
            y_left_right = (
                torch.tensor(
                    self.random_engine_boundary.random(n=self.n_boundary_left_right)[
                        :, 0
                    ],
                    device=self.device,
                    dtype=torch.float32,
            ...
            x_left_right = torch.cat(
                [
                    self.x_start
                    * torch.ones(self.n_boundary_left_right // 2, device=self.device),
                    self.x_end
                    * torch.ones(
                        self.n_boundary_left_right - self.n_boundary_left_right // 2,
                        device=self.device,
                    ),
                ]
            )
            ...
            re = torch.stack(
                [
                    u_func_torch(x_b, y_b),
                    v_func_torch(x_b, y_b),
                    p_func_torch(x_b, y_b),
                ],
                dim=-1,
            )
            return x_b, y_b, re
...
class KovasznaySamplerMC(KovasznaySamplerBase):
    ...
    def _sample_boundary(self):
        with torch.no_grad():
            x_top_bottom = (
                torch.rand(self.n_boundary_top_bottom, device=self.device)
                * (self.x_end - self.x_start)
                + self.x_start
            )
            y_top_bottom = torch.cat(
            ...
            y_left_right = (
                torch.rand(self.n_boundary_left_right, device=self.device)
                * (self.y_end - self.y_start)
                + self.y_start
            )
            x_left_right = torch.cat(
...
) -> KovasznaySamplerBase:
    if data_sampler == "latin_hypercube":
        return KovasznaySamplerLH(
            n_internal,
            n_boundary,
            device,
            update_data,
...
class KovasznayValidationDataSet:
    ...
    def __init__(
        ...
        self.x, self.y = np.meshgrid(
            np.linspace(x_start, x_end, n_point_x),
            np.linspace(y_start, y_end, n_point_y),
        )
        self.x = self.x.reshape(-1)
        self.y = self.y.reshape(-1)
        self.uvp = np.stack(
            [
                u_func_np(self.x, self.y),
                v_func_np(self.x, self.y),
                p_func_np(self.x, self.y),
            ],
            axis=-1,
        )
...
class KovasznayValidationDataLoader:
    ...
    def __init__(
        self,
        validation_dataset: KovasznayValidationDataSet,
        device: Union[str, torch.device] = "cuda:0",
    ) -> None:
        self.fake_data = [0]
