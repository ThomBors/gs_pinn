...
class SchrodingerSamplerLH(SchrodingerSamplerBase):
    ...
    def _sample_boundary(self):
        with torch.no_grad():
            t_left = (
                torch.tensor(
                    self.random_engine_boundary.random(n=self.n_boundary_left)[:, 0],
                    device=self.device,
                    dtype=torch.float32,
                )
...
class SchrodingerSamplerMC(SchrodingerSamplerBase):
    ...
    def _sample_boundary(self):
        with torch.no_grad():
            t_left = (
                torch.rand(self.n_boundary_left, device=self.device)
                * self.simulation_time
            )
            t_right = t_left
            x_left = torch.ones(self.n_boundary_left, device=self.device) * self.x_start
...
class SchrodingerValidationDataLoader:
    ...
    def __init__(
        self,
        validation_dataset: SchrodingerValidationDataSet,
        device: Union[str, torch.device] = "cuda:0",
    ) -> None:
        self.fake_data = [0]
        with torch.no_grad():
            x, t = np.meshgrid(
                validation_dataset.x.flatten(), validation_dataset.t.flatten()
            )
