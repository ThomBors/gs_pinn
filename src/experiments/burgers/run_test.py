...
def run_test(
    ...
    with torch.no_grad():
        xs_test_torch = (
            torch.from_numpy(x_test).float().to(device).unsqueeze(0).repeat(n_t, 1)
        )
        ts_test_torch = (
            torch.from_numpy(t_test).float().to(device).unsqueeze(1).repeat(1, n_x)
        )
        prediction = network(xs_test_torch, ts_test_torch)
    mse = (simulation_data - prediction) ** 2
    mse_value = torch.mean(mse)
