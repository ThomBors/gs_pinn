...
class BurgersSamplerBase:
    ...
    def __init__(
        ...
        x_end: float = X_END,
        ini_boundary: Callable = INITIAL_BOUNDAR,
    ...
    def _sample_initial_boundary(self):
        with torch.no_grad():
            ...
            return (
                torch.cat([x_initial, x_boundary]),
                torch.cat([t_initial, t_boundary]),
                torch.cat([value_initial, value_boundary]),
            )
...
class BurgersSamplerLH(BurgersSamplerBase):
    ...
    def __init__(
        ...
        x_start: float = X_START,
        x_end: float = X_END,
        ini_boundary: Callable[..., Any] = INITIAL_BOUNDAR,
        simulation_time: float = SIMULATION_TIME,
    ) -> None:
    ...
    def _sample_boundary(self):
        with torch.no_grad():
            x_boundary = torch.cat(
                [
                    self.x_start * torch.ones(self.n_boundary // 2, device=self.device),
                    self.x_end * torch.ones(self.n_boundary // 2, device=self.device),
                ]
            )
            t_boundary = torch.tensor(
                self.random_engine_initial.random(n=self.n_boundary)[:, 0],
                device=self.device,
                dtype=torch.float32,
            )
