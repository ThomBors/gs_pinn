...
from src.methods.grad_operator import *
...
class PCGrad(WeightMethod):
    ...
    def _set_pc_grads(self, losses, shared_parameters, task_specific_parameters=None):
        ...
        # task specific part
        if task_specific_parameters is not None and len(task_specific_parameters) > 0:
    ...
    def backward(
        ...
        **kwargs,
        ...
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return torch.sum(losses), {}  # NOTE: to align with all other weight methods
...
class GradDrop(WeightMethod):
    ...
    def backward(
        ...
        **kwargs,
        ...
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return torch.sum(losses), None  # NOTE: to align with all other weight methods
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            ...
            for p in shared_parameters:
                p.grad = None
        g, aligned_grads, weights = self.alignedmtl(
            grads, scale_mode=self.scale_mode, rescale=1
        )
        self.overwrite_grad(shared_parameters, g, grad_dims)
...
class OWLog(WeightMethod):
    """UW-SO-inspired multi-task learning with learnable temperature `t` to optimize task weights."""
    ...
    def get_weighted_loss(self, losses, shared_parameters, **kwargs):
        ...
        # Calculate inverse losses, then apply tempered softmax for task weights
        inv_losses = 1.0 / losses  # Detach to prevent gradients on `losses`

        # # Apply Log-Sum-Exp normalization to compute weights
        log_losses = torch.log1p(torch.abs(inv_losses))  # log(1 + |L_k|)
        lse = log_losses.sum()  # Sum of log(1 + |L_j|) for all tasks

        # # Calculate weights
        weights_loss = F.softmax(log_losses)  # Normalize

        loss = weights_loss * losses
        ...
        # self.update(loss)
        return loss.detach(), {
            "real loss loss": losses.sum().detach(),
...
class DynamicWeightAverage(WeightMethod):
    def __init__(self, n_tasks: int, device: torch.device, temp: float = 2.0):
        super().__init__(n_tasks, device=device)
        self.temp = temp
        self.device = device
        ...
        # store only what is needed: last 2 windows
        self.register_buffer("cost_history", torch.zeros(2, n_tasks, device=device))

        self.task_weights = torch.ones(n_tasks, device=device)

    @torch.no_grad()
    def update_costs(self, current_costs: torch.Tensor):
        """
        current_costs: shape [n_tasks]
        """
        self.cost_history[0] = self.cost_history[1].clone()
        self.cost_history[1] = current_costs.detach()

        self.running_iterations += 1

    def get_weighted_loss(self, losses: torch.Tensor):
        """
        losses: Tensor shape [n_tasks]
        """

        # warmup: equal weights
        if self.running_iterations < 2:
            self.task_weights = torch.ones_like(losses, device=self.device)
        else:
            L_prev = self.cost_history[0].clamp_min(1e-12)
            L_recent = self.cost_history[1]
            ...
            self.task_weights = self.n_tasks * exp_w / exp_w.sum()

        loss = torch.dot(losses, self.task_weights)

        return loss, {"weights": self.task_weights.detach().cpu()}



class DynamicWeightAverage(WeightMethod):
    """Dynamic Weight Average from `End-to-End Multi-Task Learning with Attention`.
    Modification of: https://github.com/lorenmt/mtan/blob/master/im2im_pred/utils.py#L128 
    """
    ...
    def __init__(
        ...
        self.running_iterations = 0
        self.costs = torch.ones(
            (iteration_window * 2, n_tasks),
            dtype=torch.float32,
            device=device
        )
        self.cut = torch.tensor([0.1, 0.9], requires_grad=True)
    ...
    def get_weighted_loss(self, losses, **kwargs):
        ...
        idx = self.running_iterations % (2 * self.iteration_window)

        # store current losses
        self.costs[idx] = losses.detach()

        self.running_iterations += 1

        # warmup: no DWA yet
        if self.running_iterations < 2 * self.iteration_window:
            self.task_weights = torch.ones_like(losses, device=self.device)

        else:
            recent = self.costs[self.iteration_window:]
            prev = self.costs[:self.iteration_window]

            L_recent = recent.mean(dim=0)
            L_prev = prev.mean(dim=0).clamp_min(1e-12)
        ...
        return loss, {"weights": self.task_weights}
...
class PAMGS(WeightMethod):
    """Physics Momentum Gradient Surgery """
    ...
    def _conflict_grads(self, grads: torch.Tensor):

        # Cosine similarity matrix: [T, T]
        cos_sim_matrix = grads.t() @ grads

        # Extract upper triangle (excluding diagonal)
        triu_mask = torch.triu(
            torch.ones(self.n_tasks, self.n_tasks, device=grads.device), diagonal=1
        ).bool()
        cos_sims = cos_sim_matrix[triu_mask]

        avg_cos_sim = cos_sims.mean()

        return avg_cos_sim.detach()
...
class ConFIG(WeightMethod):
    ...
    def _set_config_grads(
        self, losses, shared_parameters, task_specific_parameters=None
    ):
        # shared part
        shared_grads = []
        for l in losses:
    ...
    def _process_grads(self, grads: List[Tuple[torch.Tensor]]):
        ...
        merged_grad = self._grad_vector2tensor(
            template_grad, self.config_operator.calculate_gradient(grad_vecs)
        )
        return merged_grad
